...
exports.processTextFromPositions = processTextFromPositions;
/**
 * Process text positions into a final string with proper spacing and line breaks
 */
function processTextFromPositions(positions) {
    // IMPORTANT:
    ...
    for (const pos of positions) {
        ...
        if (lastY !== null && Math.abs(pos.y - lastY) > 5) {
            ...
            const charSpacing = pos.charSpacing !== undefined ? pos.charSpacing : lastCharSpacing;
            const wordSpacing = pos.wordSpacing !== undefined ? pos.wordSpacing : lastWordSpacing;
            ...
            if (isLastSingleChar && isCurrentSingleChar) {
                ...
                const minGapForSpace = Math.max(wordSpacing * 0.8, // 80% of word spacing (balanced)
                ...
                const minGapForSpace = Math.max(charSpacing * 1.2, // At least 1.2x character spacing (allows more spaces)
        ...
        if (pos.charSpacing !== undefined) {
            lastCharSpacing = pos.charSpacing;
        }
        if (pos.wordSpacing !== undefined) {
            lastWordSpacing = pos.wordSpacing;
        }
    }
...
function cleanText(text) {
    ...
    finalText = finalText.replace(/([a-zA-ZäöåÄÖÅ]{2,}) ([a-zA-ZäöåÄÖÅ]) ([a-zA-ZäöåÄÖÅ]{2,})/g, (match, word1, letter, word2) => {
    ...
    finalText = finalText.replace(/([a-zA-ZäöåÄÖÅ]{10,})([a-zA-ZäöåÄÖÅ]{8,})/g, (match, word1, word2) => {
