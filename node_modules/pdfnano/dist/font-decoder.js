...
class FontDecoder {
    ...
    getFont(fontDict) {
        ...
        if (!(subtype instanceof objects_1.PDFName)) {
            return null;
        }
        const fontType = subtype.name.replace('/', '');
        const baseFont = dict.get('BaseFont');
        const fontName = baseFont instanceof objects_1.PDFName ? baseFont.name.replace('/', '') : 'Unknown';
        // Get encoding information
        const encoding = dict.get('Encoding');
        let encodingName = null;
        let customEncoding = null;
        if (encoding instanceof objects_1.PDFName) {
            encodingName = encoding.name.replace('/', '');
        }
        else if (encoding instanceof objects_1.PDFDictionary) {
        ...
        let unicodeMap = null;
        if (toUnicode instanceof objects_1.PDFStream) {
            unicodeMap = this.parseToUnicode(toUnicode);
        }
        else if (toUnicode instanceof objects_1.PDFReference) {
            const obj = this.pdfStructure.getObject(toUnicode.objectNumber, toUnicode.generation);
            if (obj instanceof objects_1.PDFStream) {
        ...
        if (flags instanceof objects_1.PDFNumber) {
        ...
        if (fontDescriptor instanceof objects_1.PDFDictionary || fontDescriptor instanceof objects_1.PDFReference) {
            isEmbedded = true;
        }
        ...
        let gidToUnicode = null;
        if (isCIDFont) {
            try {
                const descendants = dict.get('DescendantFonts');
                if (descendants instanceof objects_1.PDFArray && descendants.length > 0) {
                    const first = descendants.get(0);
                    const descendantObj = first instanceof objects_1.PDFReference
                        ? this.pdfStructure.getObject(first.objectNumber, first.generation)
                        : first;
                    if (descendantObj instanceof objects_1.PDFDictionary) {
     ...
     */
    buildGidToUnicodeFromCIDFontType2(descendantFont) {
        const subtype = descendantFont.get('Subtype');
        if (!(subtype instanceof objects_1.PDFName) || subtype.name !== '/CIDFontType2')
            return null;
        ...
        if (!(cidToGidMap instanceof objects_1.PDFName) || cidToGidMap.name !== '/Identity')
            return null;
        const fd = descendantFont.get('FontDescriptor');
        const fdObj = fd instanceof objects_1.PDFReference ? this.pdfStructure.getObject(fd.objectNumber, fd.generation) : fd;
        if (!(fdObj instanceof objects_1.PDFDictionary))
            return null;
        const ff2 = fdObj.get('FontFile2');
        const ff2Obj = ff2 instanceof objects_1.PDFReference ? this.pdfStructure.getObject(ff2.objectNumber, ff2.generation) : ff2;
        if (!(ff2Obj instanceof objects_1.PDFStream))
            return null;
        const fontData = ff2Obj.getDecodedData();
        if (!Buffer.isBuffer(fontData) || fontData.length < 12)
            return null;
     ...
     */
    parseTrueTypeCmapGidToUnicode(fontData) {
        const readU16 = (off) => fontData.readUInt16BE(off);
        const readI16 = (off) => fontData.readInt16BE(off);
        const readU32 = (off) => fontData.readUInt32BE(off);
        ...
        if (cmapOffset < 0 || cmapOffset + 4 > fontData.length)
            return null;
        ...
        if (subStart + 2 > fontData.length)
            return null;
        const format = readU16(subStart);
        const gidToUni = new Map();
        const setIfAbsent = (gid, codePoint) => {
            if (gid === 0)
                return;
            if (!gidToUni.has(gid)) {
        ...
        if (format === 4) {
            if (subStart + 14 > fontData.length)
                return null;
            ...
            if (glyphArrayOff > subStart + length || glyphArrayOff > fontData.length)
                return null;
            for (let seg = 0; seg < segCount; seg++) {
                ...
                for (let code = startCode; code <= endCode; code++) {
                    ...
                    if (idRangeOffset === 0) {
                        ...
                        if (glyphIndexAddr + 2 > fontData.length)
                            continue;
                    ...
                    if (gid !== 0)
                        setIfAbsent(gid, code);
                }
            }
            return gidToUni;
        }
        if (format === 12) {
            if (subStart + 16 > fontData.length)
                return null;
            const nGroups = readU32(subStart + 12);
            let grpOff = subStart + 16;
            for (let i = 0; i < nGroups; i++) {
                if (grpOff + 12 > fontData.length)
                    break;
    ...
    parseEncodingDict(encodingDict) {
        ...
        if (differences instanceof objects_1.PDFArray) {
            let currentCode = 0;
            for (let i = 0; i < differences.length; i++) {
                const item = differences.get(i);
                if (item instanceof objects_1.PDFNumber) {
                    // Set current code
                    currentCode = item.value;
                }
                else if (item instanceof objects_1.PDFName) {
                    // Add name to mapping
     ...
     */
    parseToUnicode(toUnicodeStream) {
        const result = new Map();
        try {
            // Get stream data
            ...
            while ((match = bfrangeRegex.exec(streamData)) !== null) {
                ...
                while (idx + 2 < tokens.length) {
                    ...
                    if (!startTok.startsWith('<') || !endTok.startsWith('<'))
                        continue;
